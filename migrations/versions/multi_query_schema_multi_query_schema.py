...
Revision ID: multi_query_schema
...
revision: str = 'multi_query_schema'
...
def upgrade() -> None:
    """Create multi-query strategy tables."""
    # Chunks table
    op.create_table(
        'multi_query_chunks',
    ...
    op.create_table(
        ...
        sa.Column('chunk_id', sa.String(255), sa.ForeignKey('multi_query_chunks.chunk_id')),
...
def downgrade() -> None:
    """Drop multi-query strategy tables."""
    op.drop_table('multi_query_vectors')
    op.drop_table('multi_query_chunks')
    op.drop_table('multi_query_metadata')
