...
Revision ID: self_reflective_schema
...
revision: str = 'self_reflective_schema'
...
def upgrade() -> None:
    """Create self-reflective RAG strategy tables."""
    # Chunks table
    op.create_table(
    ...
    op.create_table(
        ...
        sa.Column('chunk_id', sa.String(255), sa.ForeignKey('self_reflective_chunks.chunk_id')),
...
def downgrade() -> None:
    """Drop self-reflective RAG strategy tables."""
    op.drop_table('self_reflective_vectors')
    op.drop_table('self_reflective_chunks')
    op.drop_table('self_reflective_metadata')
