...
class StrategyPairManager:
    ...
    def __init__(
        ...
        # Try to find primary database service for migration checks
        try:
            db_service = None
            for db_name in ['db_main', 'db1', 'main-postgres']:
                try:
                    db_service = service_registry.get(db_name)
                    if db_service:
                        break
                except Exception:
                    continue
            ...
            if db_service:
                ...
                logger.warning("No primary database service found ('db_main', 'db1'). Migration checks will be skipped.")
    ...
    def _create_strategy(self, config: Dict[str, Any], is_indexing: bool) -> Any:
        ...
        try:
            ...
            raise ConfigurationError(f"Failed to instantiate strategy '{strategy_name}': {e}")
