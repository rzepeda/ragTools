...
class SelfReflectiveRAGStrategy:
    ...
    def __init__(
        ...
        Args:
            base_retrieval_strategy: Base strategy to wrap (must have retrieve() method)
            llm_service: LLM service for grading and refinement
            config: Configuration dictionary
        """
        self.base_strategy = base_retrieval_strategy
        self.llm_service = llm_service

        # Initialize components
        config = config or {}
        self.grader = ResultGrader(llm_service, config)
        self.refiner = QueryRefiner(llm_service, config)
