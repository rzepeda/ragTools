"""
Integration tests for reranking-pair strategy configuration.
Tests two-stage retrieval with cross-encoder reranking.
"""
import pytest
...
@pytest.fixture
def mock_registry():
    """Create mock service registry for reranking strategy."""
    try:
    ...
    reranker_service.rerank = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'reranked content', 'score': 0.95, 'metadata': {}}
    ])
    
    # Mock Database Service
    db_service = Mock()
    db_service.get_chunks_for_documents = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'reranking content', 'metadata': {}}
    ])
    db_service.store_chunks = AsyncMock()
    db_service.search_chunks = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'reranking content', 'score': 0.80, 'metadata': {}},
        {'id': 'chunk2', 'text': 'other content', 'score': 0.75, 'metadata': {}}
    ])
    ...
    registry._instances["reranker_local"] = reranker_service
...
@pytest.mark.asyncio
async def test_reranking_pair_loading(mock_registry):
    ...
    with patch('rag_factory.config.strategy_pair_manager.MigrationValidator') as MockValidator:
        ...
        assert retrieval.deps.reranker_service is not None
        assert retrieval.deps.database_service is not None
        
        # Test Indexing
        docs = [{'id': 'doc1', 'text': 'Sample reranking text'}]
