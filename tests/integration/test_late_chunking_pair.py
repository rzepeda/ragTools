...
@pytest.fixture
def mock_registry():
    """Create mock service registry for late chunking strategy."""
    try:
    ...
    db_service.get_chunks_for_documents = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'late chunking content', 'metadata': {'position': 0}}
    ])
    db_service.store_chunks = AsyncMock()
    db_service.search_chunks = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'late chunking content', 'score': 0.86, 'metadata': {}}
    ])
...
@pytest.mark.asyncio
async def test_late_chunking_pair_loading(mock_registry):
    ...
    with patch('rag_factory.config.strategy_pair_manager.MigrationValidator') as MockValidator:
        ...
        chunks = await retrieval.retrieve("late chunking query", retrieval_context)
        
        assert len(chunks) >= 1
        assert chunks[0].text == "late chunking content"
