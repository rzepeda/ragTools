...
@pytest.fixture
def mock_registry():
    """Create mock service registry for query expansion strategy."""
    try:
    ...
    llm_service = Mock()
    llm_service.generate = AsyncMock(return_value="Expanded hypothetical document about the query topic")
    llm_service.agenerate = AsyncMock(return_value="Expanded hypothetical document about the query topic")
    
    # Mock Database Service
    db_service = Mock()
    db_service.get_chunks_for_documents = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'query expansion content', 'metadata': {}}
    ])
    db_service.store_chunks = AsyncMock()
    db_service.search_chunks = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'query expansion content', 'score': 0.88, 'metadata': {}}
    ])
...
@pytest.mark.asyncio
async def test_query_expansion_pair_loading(mock_registry):
