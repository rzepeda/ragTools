"""
Integration tests for self-reflective-pair strategy configuration.
Tests self-correcting retrieval with quality assessment.
"""
import pytest
...
import rag_factory.strategies.self_reflective.strategy


@pytest.fixture
def mock_registry():
    """Create mock service registry for self-reflective strategy."""
    try:
    ...
    # Mock LLM Service for reflection
    llm_service = Mock()
    llm_service.generate = AsyncMock(return_value="Quality: High\nRelevance: 0.95\nRefined query: improved query")
    llm_service.agenerate = AsyncMock(return_value="Quality: High\nRelevance: 0.95\nRefined query: improved query")
    
    # Mock Database Service
    db_service = Mock()
    db_service.get_chunks_for_documents = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'self-reflective content', 'metadata': {}}
    ])
    db_service.store_chunks = AsyncMock()
    db_service.search_chunks = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'self-reflective content', 'score': 0.89, 'metadata': {}}
    ])
...
@pytest.mark.asyncio
async def test_self_reflective_pair_loading(mock_registry):
    ...
    with patch('rag_factory.config.strategy_pair_manager.MigrationValidator') as MockValidator:
        ...
        # SelfReflectiveRAGStrategy is a full RAG strategy
        ...
        # Test Indexing
        docs = [{'id': 'doc1', 'text': 'Sample self-reflective text'}]
