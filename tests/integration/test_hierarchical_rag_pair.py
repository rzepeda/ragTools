...
@pytest.fixture
def mock_registry():
    """Create mock service registry for hierarchical strategy."""
    try:
...
@pytest.mark.asyncio
async def test_hierarchical_rag_pair_loading(mock_registry):
    ...
    with patch('rag_factory.config.strategy_pair_manager.MigrationValidator') as MockValidator:
        ...
        chunks = await retrieval.retrieve("hierarchical query", retrieval_context)
        
        assert len(chunks) >= 1
        assert chunks[0].text == "hierarchical content"
