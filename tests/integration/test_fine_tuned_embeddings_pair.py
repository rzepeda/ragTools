...
@pytest.fixture
def mock_registry():
    """Create mock service registry for fine-tuned embeddings strategy."""
    try:
    ...
    # Mock Fine-tuned Embedding Service (custom dimensions)
    embedding_service = Mock()
    embedding_service.embed = AsyncMock(return_value=[0.1] * 768)  # Custom dimension
    embedding_service.embed_batch = AsyncMock(return_value=[[0.1] * 768])
    embedding_service.get_dimension.return_value = 768
    
    # Mock Database Service
    db_service = Mock()
    db_service.get_chunks_for_documents = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'fine-tuned content', 'metadata': {}}
    ])
    db_service.store_chunks = AsyncMock()
    db_service.search_chunks = AsyncMock(return_value=[
        {'id': 'chunk1', 'text': 'fine-tuned content', 'score': 0.93, 'metadata': {}}
    ])
    ...
    registry._instances["embedding_finetuned"] = embedding_service
...
@pytest.mark.asyncio
async def test_fine_tuned_embeddings_pair_loading(mock_registry):
